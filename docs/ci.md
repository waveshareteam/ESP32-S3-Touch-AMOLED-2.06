...
Checked-in factory binaries under `FirmWare/` are recovery or release artifacts, not source projects. They are documented and excluded from build discovery. A lightweight quality and scope job is always visible; complete documentation-only changes skip expensive example builds by design, while unavailable diff data fails closed.

...
#define App_LAUNCHER_SPOT_INACTIVE_STATE     LV_STATE_DEFAULT
#define App_LAUNCHER_SPOT_ACTIVE_STATE       LV_STATE_USER_1
...
bool AppLauncher::begin(lv_obj_t *parent)
{
    ...
    vector <AppLauncherMixObject_t> mix_objs;
...
bool AppLauncher::addIcon(uint8_t page_index, const AppLauncherIconInfo_t &info)
{
    int table_last_page_index = _table_current_page_index;
    AppLauncherMixIcon_t mix_icon = {};
    ...
    auto res = _id_mix_icon_map.insert(pair<int, AppLauncherMixIcon_t>(info.id, mix_icon));
                                ...
                                AppLauncherData &data)
{
    ...
    // ESP_UTILS_CHECK_VALUE_RETURN(data.indicator.main_gap.column, 1, parent_w, false,
    //                                  "Invalid spot main layout column pad");
    // ESP_UTILS_CHECK_VALUE_RETURN(data.indicator.main_gap.row, 0, parent_h, false,
    //                                  "Invalid spot main layout row pad");
    // Icon
...
bool AppLauncher::createMixObject(ESP_Brookesia_LvObj_t &table_obj, ESP_Brookesia_LvObj_t &indicator_obj,
                                  vector<AppLauncherMixObject_t> &mix_objs)
{
...
bool AppLauncher::destoryMixObject(uint8_t index, vector <AppLauncherMixObject_t> &mix_objs)
{
...
bool AppLauncher::updateMixByNewData(uint8_t index, vector<AppLauncherMixObject_t> &mix_objs)
{
    ...
    lv_obj_set_style_bg_color(spot_obj.get(), lv_color_hex(_data.indicator.spot_active_background_color.color),
                              App_LAUNCHER_SPOT_ACTIVE_STATE);
    ...
    lv_obj_set_style_bg_color(spot_obj.get(), lv_color_hex(_data.indicator.spot_inactive_background_color.color),
                              App_LAUNCHER_SPOT_INACTIVE_STATE);
...
bool AppLauncher::updateActiveSpot(void)
{
    ...
    for (size_t i = 0; i < _mix_objs.size(); i++) {
        if ((int)i == _table_current_page_index) {
            lv_obj_add_state(_mix_objs[i].spot_obj.get(), App_LAUNCHER_SPOT_ACTIVE_STATE);
            lv_obj_set_size(_mix_objs[i].spot_obj.get(), _data.indicator.spot_active_size.width,
                            _data.indicator.spot_active_size.height);
        } else {
            lv_obj_clear_state(_mix_objs[i].spot_obj.get(), App_LAUNCHER_SPOT_ACTIVE_STATE);
...
bool AppLauncher::updateByNewData(void)
{
    ...
    lv_obj_align_to(
        _table_obj.get(), _main_obj.get(), toLvAlign(_data.table.align.type),
        _data.table.align.offset_x, _data.table.align.offset_y
    );
    // Indicator
    lv_obj_set_size(_indicator_obj.get(), _data.indicator.main_size.width, _data.indicator.main_size.height);
    lv_obj_align_to(
        _indicator_obj.get(), _main_obj.get(), toLvAlign(_data.indicator.main_align.type),
        _data.indicator.main_align.offset_x, _data.indicator.main_align.offset_y
    );
    lv_obj_set_style_pad_column(_indicator_obj.get(), _data.indicator.main_gap.column, 0);
    // Mix
    for (size_t i = 0; i < _mix_objs.size(); i++) {
